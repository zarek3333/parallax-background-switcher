...
class BackgroundSwitcher extends Backbone.Controller {
  ...
  get config() {
    return Adapt.course.get('_parallaxbgSwitcher');
  }

  onDataReady() {
    this.isMuted = this.config?._isMuted ?? false;
    const hasBackgroundSwitcher = data.some(model => model.get('_parallaxbgSwitcher')?._isActive === true);
  ...
  onPageViewPostRender({ model }) {
    if (!model.get('_parallaxbgSwitcher')?._isActive) return;
  ...
  set lowBandwidth(value) {
    ...
    Adapt.trigger('parallaxbgSwitcher:lowBandwidth', this._lowBandwidth);
  }
  ...
  get maxVideos() {
    ...
    const maxVideos = contentObjects.reduce((memo, contentobject) => {
      const videoBlocks = contentobject.findDescendantModels('block').filter((block) => {
        const config = block.get('_parallaxbgSwitcher');
        return config?.mobileSrc !== "" && config?.mobileSrc?.includes('.mp4'); //MS fix config?._isActive !== false
      });
  ...
  get images() {
    ...
    const images = contentObjects.reduce((images, contentobject) => {
      contentobject.findDescendantModels('block').forEach((block) => {
        const config = block.get('_parallaxbgSwitcher');
        //MS fix if (config?._isActive === false) return;
        if (config?.src == "") return;
        if (config?.src && !config?.src?.includes('.mp4')) images.push(config.src);
        if (config?.src) images.push(config.src);
      });
      return images;
    }, []);
    return images;
...
export default (Adapt.parallaxbgSwitcher = new BackgroundSwitcher());
